...
const pageSize = 25;
function paging(query: Filters) {
	const page = query.page ?? 1;
	return { skip: (page - 1) * pageSize, take: pageSize };
}
function result<T>(items: T[], total: number, query: Filters) {
	return { items, total, page: query.page ?? 1, pageSize };
}
...
export async function detail(professorId: number, classId: number) {
	...
	return prisma.classOffering.findUniqueOrThrow({
		where: { id: classId },
		select: {
			...classSelect,
			coursePlan: { select: studentDashboardPlanSelect },
		},
	});
}
export async function overview(professorId: number) {
	...
	const now = new Date();
	const [profile, classCount, students, pendingGrades, lessons] =
		await Promise.all([
			...
			prisma.academicActivitySubmission.count({
				where: { activity: scope, grade: null, submittedAt: { not: null } },
			}),
			prisma.classSession.findMany({
				where: { ...scope, startsAt: { gte: now }, status: 'scheduled' },
				select: lessonSelect,
...
export async function lessons(professorId: number, query: Filters) {
	...
	if (
		...
		(query.from &&
			...
			(Date.parse(query.to) <= Date.parse(query.from) ||
				Date.parse(query.to) - Date.parse(query.from) > 62 * 86400000))
	)
		throw new UnprocessableEntityError(
			'Informe um intervalo válido de até 62 dias.',
		);
	const where = {
		classOffering: assignedTo(professorId),
		classOfferingId: query.classId,
		...(query.from
	...
	const [items, total] = await prisma.$transaction([
		prisma.classSession.findMany({
			where,
			select: lessonSelect,
			orderBy: [{ startsAt: query.from ? 'asc' : 'desc' }, { id: 'asc' }],
			...paging(query),
		}),
...
export async function activities(professorId: number, query: Filters) {
	...
	const [items, total] = await prisma.$transaction([
		...
		prisma.academicActivity.count({ where }),
	]);
	return result(items, total, query);
}
export async function materials(professorId: number, query: Filters) {
	const where = {
		classSession: { classOffering: assignedTo(professorId) },
		title: { contains: query.q?.trim() ?? '', mode: 'insensitive' as const },
	};
	const [items, total] = await prisma.$transaction([
		prisma.classSessionMaterial.findMany({
			where,
			select: {
				id: true,
				title: true,
		...
		prisma.classSessionMaterial.count({ where }),
	]);
	return result(items, total, query);
}
