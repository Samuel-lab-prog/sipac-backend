...
const plans = [
	{
		...
		units: [
			...
			[
				'Aplicação',
				['Prática em laboratório', 'Projeto orientado', 'Revisão e avaliação'],
			],
		],
	},
	{
		...
		units: [
			[
				'Leitura crítica',
				['Contexto histórico', 'Leitura orientada', 'Debate'],
			],
...
export async function seedCoursePlans(
	prisma: PrismaClient,
	...
	for (const [index, offering] of offerings.entries()) {
		...
		const plan = await prisma.coursePlan.upsert({
			...
			create: {
				...
				methodology:
					'Aulas expositivas, prática orientada e acompanhamento contínuo.',
		...
		for (const [
			unitPosition,
			[unitTitle, topicTitles],
		] of blueprint.units.entries()) {
			const unit = await prisma.coursePlanUnit.upsert({
				...
				create: {
					...
					title: unitTitle,
					position: unitPosition + 1,
					workloadMinutes: 1800,
				},
			});
			for (const [topicPosition, topicTitle] of topicTitles.entries()) {
				await prisma.coursePlanTopic
					.upsert({
						where: {
							unitId_position: { unitId: unit.id, position: topicPosition + 1 },
						},
		...
		for (const [position, session] of sessions.entries()) {
			...
			if (topicId)
				await prisma.classSession.update({
					where: { id: session.id },
					data: { coursePlanTopicId: topicId },
				});
		}
	}
}
